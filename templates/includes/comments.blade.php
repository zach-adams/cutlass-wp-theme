...
<section id="comments">
  @if ($comments)
    ...
    <ol class="media-list">
      {{ wp_list_comments(array('walker' => new Roots_Walker_Comment), $comments) }}
    </ol>
      ...
      <nav>
        <ul class="pager">
          @if (get_previous_comments_link())
            <li class="previous">{{ previous_comments_link(__('&larr; Older comments', 'roots')) }}</li>
          @endif
          @if (get_next_comments_link())
            <li class="next">{{ next_comments_link(__('Newer comments &rarr;', 'roots')) }}</li>
          @endif
        </ul>
      </nav>
    @endif
...
<section id="respond">
  @if (comments_open())
    <h3>{{ comment_form_title(__('Leave a Reply', 'roots'), __('Leave a Reply to %s', 'roots')) }}</h3>
      ...
      <form action="{{ get_option('siteurl') }}/wp-comments-post.php" method="post" id="commentform">
        @if (is_user_logged_in())
          <p>
            ...
            <a href="{{ wp_logout_url(get_permalink()) }}" title="{{ _e('Log out of this account', 'roots') }}">{{ _e('Log out &raquo;', 'roots') }}</a>
          </p>
        @else
          @define $req = get_option('require_name_email')
          <div class="form-group">
            <label for="author">{{ _e('Name', 'roots'); if ($req) _e(' (required)', 'roots') }}</label>
            <input type="text" class="form-control" name="author" id="author" size="22" @if($req) echo 'aria-required="true"'; @endif>
          </div>
          <div class="form-group">
            <label for="email">{{ _e('Email (will not be published)', 'roots'); if ($req) _e(' (required)', 'roots') }}</label>
        ...
        <p><input name="submit" class="btn btn-primary" type="submit" id="submit" value="{{ _e('Submit Comment', 'roots') }}"></p>
        {{ comment_id_fields() }}
